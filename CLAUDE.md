...
This file contains instructions for Claude Code when working with the MCP Prometheus Documentation Server codebase.

## Project Overview

This is an MCP (Model Context Protocol) server that provides search and retrieval tools for Prometheus and Prometheus Operator documentation. It uses:
...
- Sparse Git checkout for efficient cloning of documentation directories
- Two upstream sources:
  - [`prometheus/docs`](https://github.com/prometheus/docs) — `docs/` and `blog/`
  - [`prometheus-operator/prometheus-operator`](https://github.com/prometheus-operator/prometheus-operator) — `Documentation/`
...
- Consider that both upstream sources publish under different domains
- Update URL computation if needed (`prometheus.io` vs `prometheus-operator.dev`)
- Handle Hugo shortcodes: `{{< >}}`, `{{% %}}`
- Blog posts follow the filename pattern `YYYY-MM-DD-slug.md` in `blog/posts/`
- Update cleaning logic for Hugo/markdown artefacts
...
- Ensure each source's content is attributed correctly (`source` field on documents)

## Project Structure

```
mcp-prometheus-documentation/
├── src/mcp_prometheus_documentation/
...
1. Check index statistics: `uv run prometheus-docs-index stats`
2. Rebuild the index: `uv run prometheus-docs-index index --rebuild`
...
The `martoc/mcp-prometheus-documentation` container image is published to Docker Hub with the documentation index pre-built.
...
|--------|-----------------|-------------|
| `prometheus` | `docs/<path>.md` | `https://prometheus.io/docs/<path>/` |
| `prometheus` | `blog/posts/YYYY-MM-DD-slug.md` | `https://prometheus.io/blog/YYYY/MM/DD/slug/` |
| `prometheus-operator` | `<path>.md` (under `Documentation/`) | `https://prometheus-operator.dev/docs/<path>/` |

The parser removes `.md` extensions and handles `index.md` / `_index.md` pages when computing URLs.

## Hugo Shortcode Cleaning

The parser handles Hugo-specific content:
- `{{< >}}` shortcode tags: removed whilst preserving inner content
- `{{% %}}` shortcode tags: removed whilst preserving inner content
- HTML comments and tags: removed

## Git Operations

The indexer uses sparse checkout to clone each upstream source:

```bash
# Prometheus docs
git clone --depth 1 --filter=blob:none --sparse --branch main \
    https://github.com/prometheus/docs.git
git sparse-checkout set docs blog
...
- **Sparse checkout**: Reduces clone size by fetching only required directories
...
2. Rebuild the index: `uv run prometheus-docs-index index --rebuild --branch main`
...
Regularly check:
- Document count: `uv run prometheus-docs-index stats`
