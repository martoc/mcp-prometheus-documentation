"""SQLite FTS5 database operations for Prometheus documentation."""
...
from mcp_prometheus_documentation.models import Document, SearchResult
...
class DocumentDatabase:
    ...
    def upsert_document(self, doc: Document) -> None:
        ...
        with self._get_connection() as conn:
            conn.execute(
                """
                INSERT INTO documents (path, title, description, section, source, url, content)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                ...
                """,
                (doc.path, doc.title, doc.description, doc.section, doc.source, doc.url, doc.content),
            )
            conn.commit()
    ...
    def search(
        ...
        Args:
            ...
            source: Optional source filter (e.g., 'prometheus', 'prometheus-operator').
